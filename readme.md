openrouter - 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

сука блядь. проблема с ошибкой была в строке vuetify/lib/styles/main.sass
поэтому пришлось ебаться хуй знает сколько времени и оборачивать все в клиентонли. это пиздец какой-то
...
- **Web3**: 
  ...
  - [ ] Подключение кошелька в login.vue: интеграция с MetaMask или другими Web3-кошельками для аутентификации через подпись, замена или дополнение email/Google OAuth.
  - [ ] Отображение Web3-данных в dashboard/profile: показ баланса TRAVEL токенов, walletAddress, истории транзакций (используя ethers.js или Moralis API), замена points на реальные токены.
- **i18n**:
  - [x] Поддержка языков: en, ru, es, zh
  - [x] Автоопределение языка
  - [x] Переключение языка в интерфейсе
  - [x] Сохранение выбранного языка при навигации по сайту
...
- **Выполнено**:
  ...
  - [x] Корректная работа локализованных URL
...
- **Сделано**:
  ...
  - i18n: Локали `en`, `ru`, `es`, `zh` с переводом, автоопределение языка, сохранение выбранного языка при навигации.
- **Осталось**:
